...
public class Flow3D : MonoBehaviour
{
    ...
    private void PositionParticles()
    {
        ...
        for (int i = 0; i < particles.Length; i++)
        {
            ...
            Vector3 curl;
            curl.x = sampleZ.derivative.x - sampleY.derivative.y; //y-z
            curl.y = sampleX.derivative.x - sampleZ.derivative.y + (1f / (1f + position.y)); //z-(x+100)
            curl.z = sampleY.derivative.x - sampleX.derivative.y; //(x+100)-y
